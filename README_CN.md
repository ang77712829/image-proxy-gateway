...
> 当前版本：v0.2.0。面向 AI Agent、NAS、New-API 和自托管工作流的本地图片/视频生成网关。默认降级链以硅基流动、魔搭为主；OpenAI-compatible、Agnes 图片、Agnes 视频等适配能力保留；Pollinations 为实验性且默认关闭。对外提供兼容 OpenAI Images 的统一接口。
...
Pollinations 为实验性渠道，默认关闭，不在默认降级链中。如需启用，在 `.env` 中设置 `BUILTIN_PROVIDER_POLLINATIONS_ENABLED=true`。
...
| 3 | Pollinations | 实验性，默认关闭 | 不在默认降级链中，需手动启用 |
...
- 默认状态：**关闭**，需手动设置 `BUILTIN_PROVIDER_POLLINATIONS_ENABLED=true` 启用
...
Web Studio v0.2.0 的”配置中心”支持内置渠道开关和密钥脱敏。Provider 管理当前支持最小创建、启用、禁用。完整 test/status/delete/sort/fallback UI 后置，不属于 v0.2.0 范围。

- 内置渠道：硅基流动、魔搭、OpenAI-compatible 图片、Agnes 图片、Agnes 视频。
- Pollinations 为实验性渠道，默认关闭。
- 自定义渠道：可通过 Web Studio 创建 `openai_image` 类型的自定义渠道，并启用/禁用。
...
- 异步视频任务：用户通过 Web Studio Jobs/Assets 查看或调用 `GET /v1/videos/{task_id}` 时，若任务已完成，网关会尝试下载到本地。
...
- 渠道管理：内置渠道启用/禁用开关；自定义 OpenAI Images 渠道支持创建和启用/禁用（完整 test/status/delete/sort 后置）；
